...
If Node is not at the recommended version use:
...
1. Serve up a local server `gulp serve:dist --codelabs-dir=codelabs`
...
Set your NodeJS version in Netlify. The solution to setting a fixed NodeJS version is :
...
- set it to your desired version (major or minor) - we recommend 12.16.2

1. Set the base directory to `docs/site`
2. Configure the build command as follows `gulp dist --codelabs-dir=codelabs && rm dist/codelabs && cp -r codelabs dist/codelabs/`
3. Set the publish directory to `docs/site/dist`

summary: How to Install Matter on Linux for ESP32 development
id: how-to-install-esp-matter-on-linux
categories: Sample
tags: matter
status: Published 
authors: MatterCoder
...
# How to Install Matter on Linux for ESP32 development
<!-- ------------------------------------------------ -->
## Overview 
Duration: 25
...
- Set up the Espressif ESP Matter SDK.
...
- Where to get the latest version of ESP-Matter and the Matter SDK 
- How to build the matter controller (chip-tool)
- How to build a sample matter app
...
You will need
- a laptop or PC running `Ubuntu 22.04` (with a Bluetooth interface for later codelabs)
...
However, Espressif have forked their own version of this repo and have bundled Matter applications and build scripts into a repo called ["esp-matter"](https://github.com/espressif/esp-matter). We will use this version of the Matter SDK for this and future codelabs. This should mean that Espressif will ensure that ESP development and the Matter release versions are compatible.
...
2. Next we will clone the ESP-matter github repo
...
3. The Matter SDK relies on a substantial number of submodules. So next we will synchronize these submodules
...
In this section we will build the matter controller tool that is provided by Project-Chip.
...
1. We will build the required sample app for testing using the following commands
...
4. If everything worked OK you should see an executable called `chip-all-clusters-app` and `chip-tool`in the `out/host` directory
...
Checkout the official documentation here: [Espressif's SDK for Matter](https://docs.espressif.com/projects/esp-matter/en/latest/esp32/index.html)

You can also checkout the official Matter SDK documentation here: [Project Chip - ConnectedHomeIp](https://github.com/project-chip/connectedhomeip/tree/master/docs)
